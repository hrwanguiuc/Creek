        ...
        if self.random:
            for epoch in range(self.epoch):
                ...
                for i in np.random.permutation(len(trainingData)):
                    ...
                    if prediction != label:
                        w1 = res[label]
                        w2 = res[prediction]
                        sig1 = sigmoid(np.dot(w1, datum))
                        sig2 = sigmoid(np.dot(w2, datum))

                        self.perceptrons[label] += alpha * sig1 * (1 - sig1) * datum
                        self.perceptrons[prediction] -= alpha * sig2 * (1 - sig2) * datum
            ...
            for epoch in range(self.epoch):
                ...
                for i in range(len(trainingData)):
                    ...
                    if prediction != label:
                        w1 = self.perceptrons[label]
                        w2 = self.perceptrons[prediction]
                        sig1 = sigmoid(np.dot(w1, datum))
                        sig2 = sigmoid(np.dot(w2, datum))

                        self.perceptrons[label] += alpha * sig1 * datum
                        self.perceptrons[prediction] -= alpha * sig2 * datum
                        num_correct_labels -= 1
