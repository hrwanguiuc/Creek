...
def heuristic(curstate,goalstate):
    ...
    for i in range(6):
        for j in range(3):
            if curstate[i][0]!= curstate[i][j+1]:
                diff+=1
    return diff
...
def Astar(cube):
    ...
    while True:  #while the queue is not empty
       ...
       #print curstate.cube.getGraph(),oldcost
        ...
        if checkstate(curcube.getGraph(),goalstate):
...
if __name__=='__main__':
    ...
    newgraph = readinputfile("Input2.3.txt")
    ...
    for i in range(num):
        printToFile("out3.2."+str(i)+".txt",moves[i])
    print (moveopration)
